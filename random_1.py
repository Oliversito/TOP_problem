...
def dist_matrix_random(positions):
    dist_random = np.zeros((num_nodes, num_nodes))
    for i in range(num_nodes):
        for j in range(num_nodes):
            dist_random[i, j] = np.sqrt((positions[i, 0] - positions[j, 0]) ** 2 + (positions[i, 1] - positions[j, 1]) ** 2)
            noise = np.random.normal(0,1)
            dist_random[i, j] += noise
    return dist_random
...
def rutas(d, tmax):
    d = dist_matrix_random(positions)
    func_ob = []
...
distancias = dist_matrix_random(positions=positions)
...
print('Tiempo: ', tiempo)
dat.to_excel('TOP1_noise1.xlsx',sheet_name='TOP1', index = False, header = False)