...
'''noise = np.random.normal(5,10)
for i in range(len(weights)):
    random_weights = weights + noise'''

def ratio_matrix(distance_matrix, weights):
    ratio_random = np.zeros((num_nodes, num_nodes))
    for i in range(num_nodes):
        for j in range(num_nodes):
            if i == j:
                ratio_random[i, j] = -1
                ...
                ratio_random[i, j] = weights[j] / (distance_matrix[i, j] + 1)
                ratio_random[i,j] += noise
    return ratio_random
...
print('Tiempo: ', tiempo)
dat.to_excel('TOP1_noise2.xlsx',sheet_name='TOP1', index = False, header = False)